...
int main() {
	...
	InitWindow(1920, 1020, "Game of Life"); //Window size, title of program
		...
		if (splashTimer > 100) 
		{
			...
			if (rulesPrompt) {
				if (menu.GetMenu()) {
					...
					DrawTextEx(font, "CLICK ANYWHERE TO CONTINUE", Vector2{ (float)GetScreenWidth() / 2 - 150, (float)GetScreenHeight() / 2 + 50 }, 32, 0, BLACK);
				}
				else {
					ClearBackground(ColorFromNormalized({ 1.0, 0.52, 0.54, 1.0 }));
					DrawTextEx(font, conwaysRules, Vector2{ 100 , 100 }, 32, 0, BLACK);
					DrawTextEx(font, warRules, Vector2{ 100 , 300 }, 32, 0, BLACK);
					DrawTextEx(font, controls, Vector2{ (float)GetScreenWidth() / 2 - 300, (float)GetScreenHeight() / 2 }, 32, 0, BLACK);
					DrawTextEx(font, "CLICK ANYWHERE TO CONTINUE", Vector2{ (float)GetScreenWidth() / 2 - 400, (float)GetScreenHeight() - 250 }, 50, 0, BLACK);
